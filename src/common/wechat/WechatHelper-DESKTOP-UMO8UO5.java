...
public class WechatHelper {//独立的 微信封装类
	...
	*/
    ...
    private byte[] httpfile(String url){
        byte[] rb;
	    try{
	        rb = InputStreamHelper.input2byte(Unirest.post(url).asBinary().getBody());
        }
	    catch (Exception e){
	        nlogger.logOut(e,url);
            rb = null;
    ...
    private byte[] posthttpfile(String url, String parameter){
	    byte[] rb;
	    try{
	        rb = InputStreamHelper.input2byte(Unirest.post(url).body(parameter).asBinary().getBody());
        }
	    catch (Exception e){
	        nlogger.logOut(e ,url + "|" + parameter);
	        rb = null;
        }
		return rb;
	}
    ...
    */
    ...
    private byte[] posthttpfile(String url, JSONObject parameter) {
        ...
        try{
            ...
            nlogger.logOut(e, url + "|" + parameter.toJSONString());
            rb = null;
        }
        return rb;
    }
